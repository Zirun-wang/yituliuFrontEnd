<template>
  <div>
    ...
    <div v-for="(item, index) in infoList" :key="index">
      {{ item.info }}：{{ item.value.toFixed(2) }}
    </div>
  ...
  methods: {
    readJson(){
        ...
        for(let i=0;i<plans.length;i++ ){
          ...
          //发电站
          var power_rooms = rooms.power;
          var powerNum = power_rooms.length;  //发电站个数
           for(let p=0; p<power_rooms.length; p++){
              var  operators =  power_rooms[p].operators;
                if("承曦格雷伊"===operators[0]||"Lancet-2"===operators[0]){
                       powerNum++;
                }
           } 
           ...
           var Kaltsit = 0;
          //控制中枢
          var control_rooms = rooms.control;
          for(let c=0; c<control_rooms.length; c++){
              var  operators =  control_rooms[c].operators;
          ...
          //办公室
          var hire_rooms = rooms.hire;
          for(let h=0; h<hire_rooms.length; h++){
              var  operators =  hire_rooms[h].operators;
                if("絮雨"===operators[0]){
          ...
          //贸易站
          var trade_rooms = rooms.trading;
          var tradeNum = trade_rooms.length;
           for(let t=0; t<trade_rooms.length; t++){
             var info = {info:"———————第"+(i+1)+"班的第"+(t+1)+"个贸易站———————",value: 0};
             this.infoList.push(info);
            
             var  operators =  trade_rooms[t].operators;
                for(let o=0; o<operators.length; o++){
                  if("黑键"===operators[o]||"夕"===operators[o]){
                       ChainLink = ChainLink+20;
                   }
                 }
          ...
          //制造站
          var manu_rooms = rooms.manufacture;
           for(let m=0; m<manu_rooms.length; m++){
             var info = {info:"———————第"+(i+1)+"班的第"+(m+1)+"个制造站———————",value: 0};
             this.infoList.push(info);
             info = {info:"思维链环",value: ChainLink};
             this.infoList.push(info);
             info = {info:"发电站个数",value: powerNum};
             this.infoList.push(info);
             var product = manu_rooms[m].product;
    ...
    cal_manu(drones,dronesIndex,index,product,manu_room,Kaltsit,powerNum,tradeNum,ChainLink,Flametail) {
      var room_char_num = manu_room.length;
      var info = { info: "房间人数", value: room_char_num };
      this.infoList.push(info);
      var manu_spd = 0; //制造效率
      var char_skill_sp = []; //特殊技能
      var char_skill_room = []; //根据设施技能
      var char_skill_variable = []; //变量类技能
      var Standardization = 0;  //标准化
      var Rhine = 0;  //莱因科技
      // var powerNum = 3; //发电站个数
      // var tradeNum = 2; //贸易站个数
      var prodLimit = 0; //仓库上限
      // var ChainLink = 60; //思维链环
      var spdByPower = false;
      // var product = 'Battle Record';
      var Knights = 0;
      ...
      for (let i = 0; i < char_skill_variable.length; i++) {
        ...
        if("配合意识"===char_skill_variable[i].skillName){
            manu_spd_item =
              ...
              if(manu_spd_item >parseFloat(char_skill_variable[i].skillLimit)){ manu_spd_item = 0.4 };  
      ...
      if("Battle Record" ===product) {
        ...
        this.capacity_exp = this.capacity_exp + capacity; 
      };  
    ...
    cal_trade(drones,dronesIndex,index,trading,Amiya,ChainLink) {
      var room_char_num = trading.length; //设施入驻人数
      var info = { info: "房间人数", value: room_char_num };
      this.infoList.push(info);
      var trade_spd = 0; //贸易效率
      var trade_spdMagnification = 1; //贸易倍率
      var char_skill_sp = []; //特殊技能
      var char_skill_variable = []; //变量类技能
      var dormLv = 20; //宿舍等级
      var meetLv = 3; //会客室等级
      var orderLimit = 10; //订单上限
      var LapplandFlag = false; //是否有拉普兰德
      var Shamare = false; //是否有巫恋
      ...
      var capacity = 427.1 *trade_spd *8; 
      this.capacity_lmd = this.capacity_lmd + capacity; 
